...
#define CONFIG_ETH0_ADDRESS	0x40 /* 6 bytes */
#define CONFIG_ETH1_ADDRESS	0x46 /* 6 bytes */
#define CONFIG_REV		0x4C /* u32 */
#define CONFIG_SDRAM_SIZE	0x50 /* u32 */
#define CONFIG_SDRAM_CONF	0x54 /* u32 */
#define CONFIG_SDRAM_MODE	0x58 /* u32 */
#define CONFIG_SDRAM_REFRESH	0x5C /* u32 */

#define CONFIG_HW_BITS		0x60 /* u32 */
#define  CONFIG_HW_USB_PORTS	0x00000007 /* 0 = no NEC chip, 1-5 = ports # */
#define  CONFIG_HW_HAS_PCI_SLOT	0x00000008
#define  CONFIG_HW_HAS_ETH0	0x00000010
#define  CONFIG_HW_HAS_ETH1	0x00000020
#define  CONFIG_HW_HAS_HSS0	0x00000040
#define  CONFIG_HW_HAS_HSS1	0x00000080
#define  CONFIG_HW_HAS_UART0	0x00000100
#define  CONFIG_HW_HAS_UART1	0x00000200
#define  CONFIG_HW_HAS_EEPROM	0x00000400
...
static void __init gmlr_init(void)
{
	...
	if ((flash = ioremap(IXP4XX_EXP_BUS_BASE_PHYS, 0x80)) == NULL)
		...
		system_rev = __raw_readl(flash + CONFIG_REV);
		hw_bits = __raw_readl(flash + CONFIG_HW_BITS);

		for (i = 0; i < ETH_ALEN; i++) {
			eth_plat[0].hwaddr[i] =
				flash_readb(flash, CONFIG_ETH0_ADDRESS + i);
			eth_plat[1].hwaddr[i] =
				flash_readb(flash, CONFIG_ETH1_ADDRESS + i);
		}
